#MASKRCNN don't need root, darken, only using single bud and angle imformation.
import cv2
import glob
import math
import numpy as np
...
if __name__ == "__main__":
    ...
    if not os.path.exists(path_save_annotation):
        os.mkdir(path_save_annotation)
    ...
    # path_an = sorted(glob.glob('/home/airlab/Desktop/IMAGE_CROP/Single_bud_dataset/datas3_singlebud/datas3/ANNOTATION/*.png'))
    ...
    for file in tqdm(path_im,total=len(path_im)):
        ...
        # for an in path_an:
        #     order_compare = str(an).split("/")[-1].split('_bud_')[0]
        #     if order_compare  == order_im_compare:
        #         file_an = cv2.imread(an)
        #         file_an = file_an[y_min_cut:y_max_cut,x_min_cut:x_max_cut]
        #         an_resize = cv2.resize(file_an,(640,456),interpolation = cv2.INTER_AREA)
        #         base_an = osp.splitext(osp.basename(an))[0].split(order_compare)[-1]
        #         filename_mask = os.path.join(path_save_annotation, f'{count}{base_an}.png')
        #         cv2.imwrite(filename_mask, an_resize)
        count += 1
    print("Done!")
